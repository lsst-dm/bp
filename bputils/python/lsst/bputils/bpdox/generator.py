...
class Generator(object):
    ...
    auto_class_regex = re.compile(
        r"\s*<\s*(?P<name>(\w+::)*\w+)\s*(?P<tparams><.*?>)?\s*(,\s*(?P<nc>(boost::)?noncopyable))?>"
    ...
    def __call__(self, input, output):
        for n, line in enumerate(input):
            ...
            while s:
                ...
                macro_output = method(macro_match.group("body"), indent=(" " * start), n=n+1)
                if head:
                    output.write(head)
                if macro_output:
    ...
    def m_scope(self, body, indent, n):
        match = self.scope_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing scope argument on line {n}.".format(n=n))
        self._scope = tuple(match.group("name").split("::"))
        return ""

    def m_doc(self, body, indent, n):
        match = self.doc_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing %%doc%% argument on line {n}.".format(n=n))
    ...
    def m_in_class(self, body, indent, n):
    ...
    def m_auto_class(self, body, indent, n):
        match = self.auto_class_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing class argument on line {n}.".format(n=n))
        node = self.index.lookup(match.group("name").split("::"), scope=self._scope)
        nc = match.group("nc") != None
        ...
        return self.formatter.getClassDeclaration(node.target, scope=self._scope, indent=indent, 
                                                  variable=variable, tparams=tparams, init=init)

    def m_auto_method(self, body, indent, n):
        if self._class is None:
            raise RuntimeError("No class active at method invokation point.")
        match = self.auto_method_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing auto_method argument on line {n}.".format(n=n))
    ...
    def m_auto_init(self, body, indent, n):
        if self._class is None:
    ...
    def m_auto_function(self, body, indent, n):
        match = self.auto_function_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing auto_function argument on line {n}.".format(n=n))
    ...
    def m_auto_enum(self, body, indent, n):
        match = self.auto_enum_regex.match(body)
        if not match:
            raise SyntaxError("Error parsing auto_enum argument on line {n}.".format(n=n))
