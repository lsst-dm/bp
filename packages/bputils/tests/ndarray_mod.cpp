...
lsst::ndarray::Array<T,N,C> makeArray(lsst::ndarray::Vector<int,N> const & shape) {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(shape);
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
...
template <int N>
lsst::ndarray::Vector<int,N> makeShape() {
...
template <typename T, int N, int C>
lsst::ndarray::Array<T,N,C> returnArray() {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(makeShape<N>());
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
        flat[n] = n;
    }
...
template <typename T, int N, int C>
bool acceptArray(lsst::ndarray::Array<T,N,C> const & p) {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(p.getShape());
    a.deep() = p;
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
...
template <typename T, int N, int C>
bool acceptArrayVal(lsst::ndarray::Array<T,N,C> p) {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(p.getShape());
    a.deep() = p;
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
...
bool extractArray(boost::python::object const & obj) {
    lsst::ndarray::Array<T,N,C> p = boost::python::extract< lsst::ndarray::Array<T,N,C> >(obj);
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(p.getShape());
    a.deep() = p;
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
...
lsst::ndarray::Vector<T,N> returnVector() {
    lsst::ndarray::Vector<T,N> r;
    for (int n=0; n<N; ++n) {
        r[n] = n;
    }
    return r;
...
template <typename T, int N>
bool acceptVector(lsst::ndarray::Vector<T,N> const & p) {
    for (int n=0; n<N; ++n) {
...
bool extractVector(boost::python::object const & obj) {
    lsst::ndarray::Vector<T,N> p = boost::python::extract< lsst::ndarray::Vector<T,N> >(obj);
    for (int n=0; n<N; ++n) {
        if (p[n] != T(n)) return false;
    }
    return true;
...
template <typename T, int N, int C>
lsst::ndarray::EigenView<T,N,C> returnEigenView() {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(makeShape<N>());
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
        flat[n] = n;
    }
    return lsst::ndarray::EigenView<T,N,C>(a);
}

template <typename T, int N, int C>
lsst::ndarray::TransposedEigenView<T,N,C> returnTransposedEigenView() {
    lsst::ndarray::Array<typename boost::remove_const<T>::type,N,N> a = lsst::ndarray::allocate(makeShape<N>());
    lsst::ndarray::Array<typename boost::remove_const<T>::type,1,1> flat = lsst::ndarray::flatten<1>(a);
    for (int n=0; n < flat.template getSize<0>(); ++n) {
        flat[n] = n;
    }
    return lsst::ndarray::TransposedEigenView<T,N,C>(a);
}
